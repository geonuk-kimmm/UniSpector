...
class SpectralGatingNetwork(nn.Module):
    def __init__(self, dim):
        ...
        self.mlp = PVT2FFN(in_features=dim, hidden_features=int(dim * 8))
    
    def forward(self, x, H, W): # [b,96,w,h]
        x = self.norm1(x)
        B, N, C = x.shape 
        #x = x.to(torch.float32) #[b,w,h,96]
        x = x.view(B, H, W, C)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        #self.freq_gate = SpectralGatingNetwork(dim=embed_dim)
