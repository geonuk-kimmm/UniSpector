...
def build_eval_dataloader(cfg, ):
    ...
    insa_test = cfg.get('InsA', {}).get('TEST', {})
    batch_size = insa_test.get('BATCH_SIZE_PER_GPU', 1)
    num_workers = insa_test.get('NUM_WORKERS', 4)
    for dataset_name in cfg['DATASETS']['TEST']:
        cfg = get_config_from_name(cfg, dataset_name)
        # adjust mapper according to dataset
        mapper = COCOInstanceNewBaselineDatasetMapper(cfg,None) #CoCoInferenceDatasetMapper(cfg, None)
        dataloaders += [build_detection_test_loader(cfg, dataset_name, batch_size=batch_size, num_workers=num_workers, mapper=mapper)]
    return dataloaders
...
def build_train_dataloader(cfg, ):
    ...
    loaders = {}
    cfg = copy.deepcopy(cfg)
    insa_train = cfg.get('InsA', {}).get('TRAIN', {})
    cfg['DATALOADER']['NUM_WORKERS'] = insa_train.get('NUM_WORKERS', 4)
